/* CS5480 Assignment 0 Extra Credit
 * Author: Spencer Lee
 * Purpose: Implements a vector of generic type using macros
 */
...
#define vector_new(T, init_capacity, dtor)                                 \
  ({                                                                       \
    /* Malloc the vector_info and data array as one contiguous block */    \
    size_t __impl_vn_mal_size =                                            \
        sizeof(vector_info) + (init_capacity) * sizeof(T);                 \
    vector_info* __impl_vn_vec = (vector_info*)malloc(__impl_vn_mal_size); \
    if (__impl_vn_vec == NULL) {                                           \
      panic("Failed to malloc");                                           \
    }                                                                      \
                                                                           \
    /* Set default values */                                               \
    __impl_vn_vec->len = 0;                                                \
    __impl_vn_vec->capacity = (init_capacity);                             \
    __impl_vn_vec->ele_dtor = (dtor);                                      \
                                                                           \
    /* __impl_vn_vec + 1 is the memory immediately after*/                 \
    /* the vector_info, i.e. the array of data */                          \
    ((vector(T))(__impl_vn_vec + 1));                                      \
  })
...
#define vector_len(self) ({ get_vector_header(self)->len; })
...
#define vector_capacity(self) ({ get_vector_header(self)->capacity; })
...
#define vector_resize(self, n)                                           \
  ({                                                                     \
    vector_info* __impl_vr_header = get_vector_header(self);             \
                                                                         \
    /* Resize only if the new capacity is greater */                     \
    if ((n) > __impl_vr_header->len) {                                   \
      /* Create a new vector with new capacity */                        \
      vector(typeof(**(self))) __impl_vr_vec =                           \
          vector_new(typeof(**(self)), (n), __impl_vr_header->ele_dtor); \
                                                                         \
      /* Copy over data to the new vector */                             \
      for (size_t __impl_vr_i = 0; __impl_vr_i < __impl_vr_header->len;  \
           __impl_vr_i++) {                                              \
        __impl_vr_vec[__impl_vr_i] = (*(self))[__impl_vr_i];             \
      }                                                                  \
      get_vector_header(&__impl_vr_vec)->len = __impl_vr_header->len;    \
                                                                         \
      /* Replace old vector with new vector */                           \
      free(__impl_vr_header);                                            \
      *(self) = __impl_vr_vec;                                           \
    }                                                                    \
...
#define vector_get(self, index)                                   \
  ({                                                              \
    if ((index) < 0 || (index) >= get_vector_header(self)->len) { \
...
#define vector_set(self, index, ...)                           \
  ({                                                           \
    vector_info* __impl_vs_header = get_vector_header(self);   \
    if ((index) < 0 || (index) >= __impl_vs_header->len) {     \
      panic("Index out of range");                             \
    }                                                          \
                                                               \
    /* Call delete function on the to-be-overridden element */ \
    if (__impl_vs_header->ele_dtor) {                          \
      __impl_vs_header->ele_dtor(*(self) + index);             \
    }                                                          \
                                                               \
    (*(self))[index] = __VA_ARGS__;                            \
    ((void)0); /* (optional) return "nothing" */               \
  })
...
#define vector_push(self, ...)                                               \
  ({                                                                         \
    vector_info* __impl_vpu_header = get_vector_header(self);                \
                                                                             \
    /* Increase capacity of vector if necessary */                           \
    if (__impl_vpu_header->len >= __impl_vpu_header->capacity) {             \
      size_t __impl_vp_cap =                                                 \
          __impl_vpu_header->capacity ? 2 * __impl_vpu_header->capacity : 1; \
      vector_resize(self, __impl_vp_cap);                                    \
      __impl_vpu_header = get_vector_header(self);                           \
    }                                                                        \
                                                                             \
    (*(self))[__impl_vpu_header->len] = __VA_ARGS__;                         \
    __impl_vpu_header->len++;                                                \
    ((void)0); /* (optional) return "nothing" */                             \
  })
...
#define vector_pop(self)                                               \
  ({                                                                   \
    vector_info* __impl_vpo_header = get_vector_header(self);          \
    bool __impl_vpo_success = __impl_vpo_header->len > 0;              \
                                                                       \
    /* Only pop if thre is an element to pop */                        \
    if (__impl_vpo_success) {                                          \
      /* Call delete function on last element */                       \
      if (__impl_vpo_header->ele_dtor) {                               \
        __impl_vpo_header->ele_dtor(*(self) + __impl_vpo_header->len); \
      }                                                                \
                                                                       \
      __impl_vpo_header->len--;                                        \
    }                                                                  \
    __impl_vpo_success;                                                \
  })
...
// vector_insert(&v, 1, 6);
// /* after: v = {3, 6, 2, 4}; */
#define vector_insert(self, index, ...)                                       \
  ({                                                                          \
    vector_info* __impl_vi_header = get_vector_header(self);                  \
    if ((index) < 0 || (index) > __impl_vi_header->len) {                     \
      panic("Index out of range");                                            \
    }                                                                         \
                                                                              \
    /* Shift existing elements to make space for new element */               \
    if (__impl_vi_header->len >= __impl_vi_header->capacity) {                \
      /* vec_resize here would perform unneccesary copies */                  \
      /* Instead create a new vector with custom copy logic */                \
      size_t __impl_vi_cap =                                                  \
          __impl_vi_header->capacity ? 2 * __impl_vi_header->capacity : 1;    \
      vector(typeof(**(self))) __impl_vi_vec = vector_new(                    \
          typeof(**(self)), __impl_vi_cap, __impl_vi_header->ele_dtor);       \
                                                                              \
      /* Copy elements over from range [0, index-1] */                        \
      for (size_t __impl_vi_i = 0; __impl_vi_i < (index); __impl_vi_i++) {    \
        __impl_vi_vec[__impl_vi_i] = (*(self))[__impl_vi_i];                  \
      }                                                                       \
                                                                              \
      /* Copy from [index...self->length-1] to [index+1...self->length] */    \
      for (size_t __impl_vi_i = __impl_vi_header->len; __impl_vi_i > (index); \
           __impl_vi_i--) {                                                   \
        __impl_vi_vec[__impl_vi_i] = (*(self))[__impl_vi_i - 1];              \
      }                                                                       \
                                                                              \
      /* Replace old vector with new vector */                                \
      free(*(self));                                                          \
      *(self) = __impl_vi_vec;                                                \
    } else { /* Shift from [index...self->length-1] to                        \
                [index+1...self->length] */                                   \
      for (size_t __impl_vi_i = __impl_vi_header->len; __impl_vi_i > (index); \
           __impl_vi_i--) {                                                   \
        (*(self))[__impl_vi_i] = (*(self))[__impl_vi_i - 1];                  \
      }                                                                       \
    }                                                                         \
                                                                              \
    (*(self))[index] = __VA_ARGS__;                                           \
    __impl_vi_header->len++;                                                  \
    ((void)0); /* (optional) return "nothing" */                              \
  })
...
#define vector_erase(self, index)                                      \
  ({                                                                   \
    vector_info* __impl_ve_header = get_vector_header(self);           \
    if ((index) < 0 || (index) >= __impl_ve_header->len) {             \
      panic("Index out of range");                                     \
    }                                                                  \
                                                                       \
    /* Call delete function on the to-be-removed element */            \
    if (__impl_ve_header->ele_dtor) {                                  \
      __impl_ve_header->ele_dtor(*(self) + (index));                   \
    }                                                                  \
                                                                       \
    /*  Shift [index+1...self->length-1] to [index...self->length-2]*/ \
    for (size_t __impl_ve_i = (index);                                 \
         __impl_ve_i + 1 < __impl_ve_header->len; __impl_ve_i++) {     \
      (*(self))[__impl_ve_i] = (*(self))[__impl_ve_i + 1];             \
    }                                                                  \
                                                                       \
    __impl_ve_header->len--;                                           \
    ((void)0); /* (optional) return "nothing" */                       \
  })
...
#define vector_free(self)                                               \
  ({                                                                    \
    vector_info* __impl_vf_header = get_vector_header(self);            \
                                                                        \
    /* Call delete function on all elements */                          \
    if (__impl_vf_header->ele_dtor) {                                   \
      for (size_t __impl_vf_i = 0; __impl_vf_i < __impl_vf_header->len; \
           __impl_vf_i++) {                                             \
        __impl_vf_header->ele_dtor(*(self) + __impl_vf_i);              \
      }                                                                 \
    }                                                                   \
                                                                        \
    free(__impl_vf_header);                                             \
  })
